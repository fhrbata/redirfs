...
enum rfs_retv dummyflt_permission(rfs_context context, struct rfs_args *args)
{
	...
	rv = rfs_get_filename(args->args.i_permission.nd->dentry,
			args->args.i_permission.nd->mnt, path, PAGE_SIZE);
	if (rv) {
...
enum rfs_retv dummyflt_open(rfs_context context, struct rfs_args *args)
{
	...
	rv = rfs_get_filename(args->args.f_open.file->f_dentry,
			args->args.f_open.file->f_vfsmnt, path, PAGE_SIZE);
	if (rv) {
		printk(KERN_ALERT "dummyflt: rfs_get_filename failed\n");
		return RFS_CONTINUE;
	}
