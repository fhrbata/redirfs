...
/**
 * redirfs_register_callback() - registers a new filter with RedirFS
 * @info:	filter description
 *
 * Description:
 *     Registers a new filter described by info with RedirFS a creates
 *     a basic sysfs interface for it. Return a pointer to the registered
 *     filter on success.
 **/
...
/**
 * redirfs_unregister_filter() - unregisters a previously registered filter
...
/**
 * redirfs_delete_filter() - removes a filter's sysfs interface
 * @filter:	pointer to the filter
 **/
...
/**
 * redirfs_set_operations() - set filter's callbacks for selected operations
 * @filter:	pointer to the filter
 * @ops:	array of redirfs_op_info containing the callbacks details
 *
 * Description:
 *     Sets filter's callbacks for the selected VFS operations. The array ops
 *     must end with {REDIRFS_OP_END, NULL, NULL}.
 **/
...
/**
 * redirfs_activate_filter() - activates filter
 * @filter:	pointer to the filter
 **/
...
/**
 * redirfs_deactivate_filter() - deactivates filter
 * @filter:	pointer to the filer
 **/
